...
def token():
    token_address = "0x79c912FEF520be002c2B6e57EC4324e260f38E50"  # this should be the address of the ERC-20 used by the strategy/vault (weth/USDC LP)
...
def whale(amount, token):
    ...
    whale = accounts.at(
        "0xE2CEc8aB811B648bA7B1691Ce08d5E800Dd0a60a", force=True
    )  # 0xE2CEc8aB811B648bA7B1691Ce08d5E800Dd0a60a, weth-usdc gauge
...
def profit_amount(token):
    profit_amount = 0.00000005 * 10 ** token.decimals()
    yield profit_amount
...
@pytest.fixture(scope="session")
def strategy_name():
    strategy_name = "StrategyVelodrome"
    yield strategy_name
...
if chain_used == 1:  # mainnet
    ...
    @pytest.fixture(scope="session")
    def to_sweep():
        # token we can sweep out of strategy (use VELO)
        yield interface.IERC20("0x3c8B650257cFb5f272f799F5e2b4e65093a11a05")
...
@pytest.fixture(scope="session")
def gauge():
    yield Contract("0xE2CEc8aB811B648bA7B1691Ce08d5E800Dd0a60a")  # weth/usdc gauge
